...
void createCSVOutputFile(std::vector<TimingInfo> &timingInfo) {
    ...
    constexpr char csvName[]{ "Midterm_Project.csv" };

    std::cout << "Writing output file: " << csvName << std::endl;
    std::ofstream csvStream{ csvName };

    csvStream << "Image#." << COMMA;
    csvStream << "Detector_Type" << COMMA;
    csvStream << "Descriptor_Type" << COMMA;
    csvStream << "Keypoints" << COMMA;
    csvStream << "Keypoints(Proceeding Vehical)" << COMMA;
    csvStream << "(Detector)Elapsed_Time" << COMMA;
    csvStream << "(Descriptor)Elapsed_Time" << COMMA;
    csvStream << "Matches" << COMMA;
